...
def plot_images(image) :
    ...
    for i, ax in enumerate(axes.flat) :
        ...
        xlabel = '{},{}'.format(row, col)       # 각 사진 밑에 표시될 숫자 묶음
        ax.set_xlabel(xlabel)                   # 사진 밑에 xlabel 표시하게 함
        ax.set_xticks([])  # Remove ticks from the plot.    # x축 눈금 변경 (공백 -> remove)
        ax.set_yticks([])                                   # y축 눈금 변경 (공백 -> remove)

    plt.show()

plt.imsave("sliced image.jpg", img[50:100, 50:100, :], cmap=plt.cm.gray)
plot_images(img)    #분할된 모습을 보여줌
