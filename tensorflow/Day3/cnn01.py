...
x_image = tf.reshape(images_batch, [-1, IMG_HEIGHT, IMG_WIDTH, NUM_CHANNEL])    # 4차원 배열
# -1은 None과 같은 의미
# -1이 있는 위치에는 batch size를 입력하는 부분임
# = 나중에 이미지를 왕창 주면 나중에 니가 그걸 세어서 입력해라 (undefined)
...
# 변수를 만든걸 또 하나의 변수에 이렇게 저장하는 이유 : training 한 후에 그 결과를 변수에 저장해서 다음에도 불러올 수 있게끔 하기 위해
...
) # h_conv1 의 shape : 50 X 60 X 60 X 32
# padding='SAME' : 그냥 0으로 채운다.

# 첫번째 convolution 을 통과한 결과를 pooling 할 것임
h_pool1 = tf.nn.max_pool(h_conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
# shape : 50 X 30 X 30 X 32

# ===Second Concolution + Pooling Layer===

w_conv2 = tf.get_variable(name='w_conv2', shape=[5, 5, 32, 64], dtype=tf.float32)
# 5X5 크기의 3채널 kernel 64개를 사용
...
# keep_prob을 입력으로 주어질 여지를 줘야하기 때문에 직접 확률을 지정하지 않고 placeholder 로 놓는다.
...
for i in range(int(len(test_features)/TEST_BSIZE)):
    ...
    loss_val, accuracy_ = sess.run([loss_mean, accuracy],
                                   feed_dict={
                                       images_batch: images_batch_val,
                                       labels_batch: labels_batch_val,
                                       keep_prob: 1.0   # test 할 때는 dropout 을 1.0으로 데이터를 모두 살려서 해야함
    })
